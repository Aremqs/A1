...
background = transform.scale(image.load("tlo.jpg"),(w_width,w_height))
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
game = True
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
hero = Player("korabl.png",10,400,60,60,16)
#-=-=-=-
...
#=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
lose = font2.render("ВИ ПРОГРАЛИ!",1,(0,0,0))
victory = font2.render("ВИ ВИГРАЛИ!",1,(250,250,250))
while game:
    ...
    if not finish:
        ...
        text1=font2.render("РАХУНОК: "+str(score),1,(1,1,1))
        window.blit(text1,(25,25))
        text2=font2.render("ПРОПУЩЕНО:"+str(lost),1,(1,1,1))
